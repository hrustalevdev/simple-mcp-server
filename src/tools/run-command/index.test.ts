...
describe("runCommand", () => {
  it("runs an allowed command and captures stdout", async () => {
    const result = await runCommand("echo hello", WHITELIST, CWD);
  ...
  it("rejects commands not in whitelist", async () => {
    await expect(runCommand("rm -rf /", WHITELIST, CWD)).rejects.toThrow(
      "command not in whitelist"
    );
  });

  it("runs node --version and returns version string", async () => {
    const result = await runCommand("node --version", WHITELIST, CWD);
    expect(result.success).toBe(true);
